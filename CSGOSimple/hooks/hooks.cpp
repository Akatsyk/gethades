...
namespace hooks
{
	...
	vfunc_hook filesystem_hook;
	vfunc_hook check_file_crcs_hook;
	//uintptr_t** test;
	...
	void initialize()
	{
		...
		//test = reinterpret_cast<uintptr_t**> ( GetModuleHandle ( L"client.dll" ) ) + 0x10A8F86C;
		...
		hlclient_hook.setup(interfaces::chl_client, xor_str("client.dll"));
		...
		clientmode_hook.setup(interfaces::client_mode, xor_str("client.dll"));
		...
		//  clientstate_hook.setup ( interfaces::client_state, xor_str ( "client.dll" ) );
		engine_client_hook.setup(interfaces::engine_client, xor_str("engine.dll"));
		// prediction.setup ( interfaces::prediction, xor_str ( "client.dll" ) );
		 //entity_renderable_hook.setup ( interfaces::dwCCSPlayerRenderablevftable, "client.dll" );
		...
		//clientstate_hook.hook_index( index::temp_entities_t, hk_temp_entities );

		setup_fire_events();

		auto tmp_addr = g_utils.pattern_scan(xor_str("client.dll"),
		...
		auto file_system = g_utils.pattern_scan(xor_str("engine.dll"), xor_str("8B 0D ? ? ? ? 8D 95 ? ? ? ? 6A 00 C6") + 0x2);

		filesystem_hook.setup(file_system);
		filesystem_hook.hook_index(128, hk_loose_file_allowed);

		auto check_file_crcs = g_utils.pattern_scan(xor_str("engine.dll"), xor_str("55 8B EC 81 EC ? ? ? ? 53 8B D9 89 5D F8 80"));
		DetourFunction((PBYTE)check_file_crcs, (PBYTE)hk_check_file_crcs_with_server);
	...
	void shutdown()
	{
		...
		//const auto c_csplayer_vtable = ( g_utils.pattern_scan ( "client.dll", "E8 ? ? ? ? C7 87 ? ? ? ? ? ? ? ? C6 87 ? ? ? ? ? C7 87 ? ? ? ? ? ? ? ?" ) + 0x52 ) + 0x2;
