...
#define add_idx(name, idx) constexpr int name##  = idx
	...
	namespace ccsplayer
	{
		add_idx(update_client_side_animations, 223);
		add_idx(is_player, 157);
	}

	namespace ccscombat_weapon
	{
		add_idx(is_weapon, 165);
		add_idx(get_cs_weapon_data, 460);
		add_idx(get_inaccuracy, 482);
		add_idx(get_spread, 452);
		add_idx(update_accuracy_penalty, 483);
	}

	namespace cattributable_item
	{
		add_idx(set_glove_model_index, 75);
		add_idx(send_view_model_matching_sequence, 246);
	}
}
...
public:
	...
	int get_sequence()
	{
		static auto get_sequence = g_netvarsys.GetOffset(xor_str("DT_BaseAnimating"), xor_str("m_nSequence"));
		return *reinterpret_cast<int*> (reinterpret_cast<uintptr_t> (this) + get_sequence);
	}

	int prev_bone_mask()
	{
		return *reinterpret_cast<int*> (reinterpret_cast<uintptr_t> (this) + 0x2698 + 0x4);
	}
...
public:
	template< typename t >
	__forceinline t& get_offset(size_t offset) {
		return *(t*)((uintptr_t)this + offset);
	}

	template< typename t >
	__forceinline void set_offset(size_t offset, const t& val) {
	...
	void set_abs_angles(const QAngle& angles)
	{
		...
		static SetAbsAnglesFn SetAbsAngles = (SetAbsAnglesFn)g_utils.pattern_scan(xor_str("client.dll"),
	...
	void set_abs_original(Vector origin)
	{
		...
		if (!SetAbsOrigin)
			SetAbsOrigin = (SetAbsOriginFn)((DWORD)g_utils.pattern_scan(xor_str("client.dll"),
...
public:
	...
	Vector m_vOrigin; //0xB0, 0xB4, 0xB8
	Vector m_vLastOrigin; //0xBC, 0xC0, 0xC4
	Vector m_vVelocityX; //0xC8
	Vector m_vVelocityY; //0xCC
	char pad5[4];
	float m_flUnknownFloat1; //0xD4 Affected by movement and direction
	char pad6[8];
	float m_flUnknownFloat2; //0xE0 //from -1 to 1 when moving and affected by direction
